...
function saveOptions() {
    ...
    const simklTokenVal = document.getElementById('simklToken').value.trim();
    ...
    if (simklTokenVal) {
        saveData.simkl_token = { access_token: simklTokenVal, created_at: Math.floor(Date.now() / 1000) };
    }
...
function restoreOptions() {
    chrome.storage.local.get(['client_id', 'client_secret', 'simkl_token', 'tmdb_api_key', 'deepseek_api_key'], (items) => {
        ...
        if (items.simkl_token) {
            document.getElementById('simklToken').value = typeof items.simkl_token === 'string' ? items.simkl_token : (items.simkl_token.access_token || '');
        }
